...
def parse_arguments():
    ...
    parser.add_argument('-u','--url',dest='url',help='The page URL')
    parser.add_argument('-w','--wordlist',dest='wordlist',help='The wordlist')
    parser.add_argument('-e','--extensions',dest='extensions',help='The extensions to probe')
    parser.add_argument('-t','--thread',dest='thread',help='The number of the working threads')
    parser.add_argument('-s','--status-codes',dest='statuscodes',help='The status codes to watch')    
    parser.add_argument('-R','--regex',dest='regex',help='Use Regex pattern to response')
    parser.add_argument('-cl','--content-length',dest='contentlength',help='The length of the reponse')
    parser.add_argument('-T','--timeout',dest='timeout',help='The timeout to wait a worker for an answer')
    parser.add_argument('-st','--send-timeout',dest='sendtimeout',help='The sending timeout between requests')
    parser.add_argument('-k','--avoid-ssl',dest='avoidssl',action='store_true',help='Avoid to verrify SSL certificate')
    parser.add_argument('-proxy',dest='proxy',help='Set up proxy. HOST:PORT')
    parser.add_argument('-rc','--response-contains',dest='rcontains',help='The response is contains a word')
    parser.add_argument('-c','--cookies',dest='cookies',help='Use Cookies for the request')
    parser.add_argument('--header',dest='header',action='append',nargs='+',help='Header fields of the requests')
    parser.add_argument('-ua','--user-agent',dest='useragent',help='The UserAgent String for use the requests')
    parser.add_argument('-o','--output',dest='output',help='Write result into outfile.')
